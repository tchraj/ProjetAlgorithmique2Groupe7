...
# ── Logging : branché sur gunicorn pour apparaître dans docker compose logs ───
gunicorn_logger = logging.getLogger('gunicorn.error')
app.logger.handlers = gunicorn_logger.handlers
app.logger.setLevel(gunicorn_logger.level if gunicorn_logger.level != 0 else logging.INFO)
app.logger.info("=== Kitchen Scheduler démarré ===")
app.logger.info(f"Algorithmes disponibles : johnson, neh, lpt, fifo, brute")
...
@app.route('/')
def index():
    app.logger.info("GET / — Page d'accueil")
    return render_template('index.html')


@app.route('/activite-debranchee')
def activite_debranchee():
    app.logger.info("GET /activite-debranchee — Activité débranchée")
    return render_template('activite_debranchee.html')


@app.route('/compare')
def compare():
    app.logger.info("GET /compare — Page de comparaison des algorithmes")
    return render_template('compare.html')


@app.route('/jouer')
def jouer():
    app.logger.info("GET /jouer — Mode humain")
    return render_template('jouer.html')


@app.route('/complexite')
def complexite():
    app.logger.info("GET /complexite — Page complexité")
    return render_template('complexite.html')


@app.route('/pedagogie')
def pedagogie():
    app.logger.info("GET /pedagogie — Page pédagogique Johnson")
...
def get_plats():
    app.logger.info("GET /api/plats — Catalogue des plats demandé")
...
def assign_plat():
    ...
    app.logger.info(f"POST /api/assign — plat={plat_id}, etape={current_etape}, algo={algorithm}")

    if plat_id not in PLATS_CATALOGUE:
        app.logger.warning(f"POST /api/assign — plat inconnu : {plat_id}")
...
def simulate_schedule():
    ...
    app.logger.info(f"POST /api/simulate — {len(plats_ids)} plats, algo={algorithm}")
    ...
    app.logger.info(f"POST /api/simulate — makespan estimé : {total_makespan}s")
...
def schedule():
    ...
    app.logger.info(f"POST /schedule — {num_workers} workers, algo={algorithm}")
    ...
    app.logger.info(f"POST /schedule — makespan={makespan}")
    return jsonify({'workers': workers, 'makespan': makespan})
...
def api_compare():
    ...
    app.logger.info(f"POST /api/compare — {len(plats_raw)} plats, algos={algos_req}, commis={nb_commis}, fours={nb_fours}")
    ...
    app.logger.info(f"POST /api/compare — meilleur algo : {meilleur_algo}, makespan={meilleur_makespan}s")
...
def list_instances():
    app.logger.info("GET /api/instances — Liste des instances demandée")
...
def get_instance(nom):
    app.logger.info(f"GET /api/instances/{nom}")
    instance = instance_manager.obtenir_instance_par_nom(nom)
    if not instance:
        app.logger.warning(f"GET /api/instances/{nom} — instance non trouvée")
...
def generate_instance():
    ...
    app.logger.info(f"POST /api/instances/generate — {nombre_plats} plats, difficulté={difficulte}")
...
def api_jouer():
    ...
    app.logger.info(f"POST /api/jouer — {len(plats_raw)} plats, ordre humain soumis")
    ...
    app.logger.info(
        f"POST /api/jouer — makespan humain={res_human['makespan']}s, "
        f"makespan Johnson={res_johnson['makespan']}s"
    )
